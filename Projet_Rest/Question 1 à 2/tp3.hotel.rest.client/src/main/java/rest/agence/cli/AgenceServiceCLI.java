...
public class AgenceServiceCLI extends AbstractMain implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		 ...
		 try {
			...
			localhostPort.put("La Verrerie",SERVICE_URL+"8080/hotelservice/api/");
			localhostPort.put("Progress",SERVICE_URL+"5555/hotelservice/api/");
			do {
				menu(); 
	...
	private void processUserInput(BufferedReader reader, String userInput, RestTemplate proxy) {
		try {
			...
			case "1": {
				...
				intervalleFloatInputProcessor = new FloatPriceInputProcessorFirstPriceMin(reader, 
				...
				prixIntervalleStockInput.addAll(intervalleFloatInputProcessor.prixIntervalle());
				...
				for (String hotelAdress : localhostPort.values()) {
					String uri = hotelAdress+"hotel/consultation/id_agence="+identifiantAgence+"/password_agence="+passwordAgenceToProxy+""
			...
			case "2": {
			    ...
			    for (String hotelAdresse : localhostPort.values()) {
					String uriNomHotel = hotelAdresse+"hotel/nomhotel"; 
					String hotelAddToList = proxy.getForObject(uriNomHotel, String.class);
					
					String uriListOffre = hotelAdresse+"hotel/chambre"; 
				...
				String offre; 
				do {
					System.out.println("Veuillez entrée un numéro d'offre parmi la liste proposée :\n"+KeyNomHotelValueIdOffre.get(hotel).toString());
			    ...
			    params.add("cryptogramme", cryptogramme);
				...
				dateInputTest(reader);
				params.add("dateArrivee", dateArriveeToProxy);
				params.add("dateDepart", dateDepartToProxy); 
				nbPersonInputTest(reader);
				params.add("nbLitsResa", Integer.toString(nbPersonToProxy));
